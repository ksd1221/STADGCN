...
class CustomLoss(nn.Module):
    def __init__(self, weight_for_zero=2.0, weight_for_non_zero=1.5):
        super(CustomLoss, self).__init__()
        self.weight_for_zero = weight_for_zero
        self.weight_for_non_zero = weight_for_non_zero
        self.mse_loss = nn.MSELoss(reduction='none')  # 기본적으로 MSE 손실을 사용하되, 각 샘플에 대한 손실을 따로 계산

    def forward(self, y_pred, y_true, mask):
        loss = self.mse_loss(y_pred, y_true)
        weights = torch.where(mask == 0, self.weight_for_zero, self.weight_for_non_zero)
        weighted_loss = loss * weights
        return weighted_loss.mean()  # 최종 손실은 가중치를 적용한 손실의 평균